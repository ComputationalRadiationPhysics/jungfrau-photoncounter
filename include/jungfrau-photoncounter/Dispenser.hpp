  ...
  Dispenser(Dispenser&& other) = default;
        ...
        -> std::tuple<std::size_t, std::future<bool>>
    {
        ...
        if (offset + TConfig::DEV_FRAMES <= data.numFrames) {
            ...
            auto result = processData(dataView,
                                      TConfig::DEV_FRAMES,
                                      flags,
                                      energy,
                                      photon,
...
private:
  ...
  std::size_t nextFree, nextFull;
    ...
    template <typename TFramePackageDetectorData,
              typename TFramePackageEnergyMap,
              typename TFramePackagePhotonMap,
              typename TFramePackageSumMap,
              typename TFramePackageEnergyValue>
    auto processData(TFramePackageDetectorData data,
                     std::size_t numMaps,
                     ExecutionFlags flags,
                     tl::optional<TFramePackageEnergyMap> energy,
                     tl::optional<TFramePackagePhotonMap> photon,
        ...
        -> std::tuple<std::size_t, std::future<bool>>
    {
        ...
        FramePackageDoubleBuffer<TAlpaka,
                                 ...
                                 decltype(dev->sum)>
            sumBuffer(
                sum, dev->sum, &dev->queue, numMaps / TConfig::SUM_FRAMES);
        FramePackageDoubleBuffer<TAlpaka,
                                 TFramePackageEnergyValue,
        ...
        typename GetDoubleBuffer<TAlpaka, ClusterView, decltype(dev->cluster)>::
    ...
    template <typename TData,
              typename TOptionalEnergy,
              typename TOptionalPhoton,
              typename TOptionalSum,
              typename TOptionalMaxValues,
              typename TOptionalClusters,
              typename TOptionalUsedClusters>
    auto enqueueKernels(DeviceData<TConfig, TAlpaka>* dev,
                        std::size_t numMaps,
                        ExecutionFlags flags,
                        TData data,
                        TOptionalEnergy energy,
                        TOptionalPhoton photon,
                        TOptionalSum sum,
                        TOptionalMaxValues maxValues,
