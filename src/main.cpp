...
auto main(int argc, char *argv[]) -> int {
  ...
  typename Config::FramePackage<typename Config::DetectorData, ConcreteAcc>
      data(fc->loadMaps<typename Config::DetectorData, ConcreteAcc>(
          ...
          //"../../../data_pool/synthetic/cluster_8.bin",
  ...
  typename Config::ClusterArray<ConcreteAcc> clusters_data(
      Config::MAX_CLUSTER_NUM_USER * data.numFrames);
  ...
  // std::ofstream maxValues_file("/bigdata/hplsim/production/"
  //                             "jungfrau-photoncounter/reference/maxValues.bin",
  //                             std::ios_base::binary);
  ...
  std::ofstream energy_file("/bigdata/hplsim/production/jungfrau-photoncounter/"
                            "reference/cluster_energy.bin",
                            std::ios_base::binary);
  ...
  //    "/bigdata/hplsim/production/jungfrau-photoncounter/reference/photon.bin",
  //    std::ios_base::binary);
  // if (!energy_file.is_open()) {
  ...
  // std::ofstream energy_file("cluster_energy_cl810000_masking_cluster3.bin",
  // std::ios_base::binary);
  ...
  while (downloaded < data.numFrames) {
    ...
    if (currently_downloaded_frames) {
      ...
      //      sizeof(typename Config::EnergyValue) *
      //      currently_downloaded_frames);
      //
  ...
  if (clusters) {
    // saveClusters<Config,
    // ConcreteAcc>("clusters_cl810000_masking_cluster3.txt", *clusters);
    saveClustersBin<Config, ConcreteAcc>(
        "/bigdata/hplsim/production/jungfrau-photoncounter/reference/"
        "clusters.bin",
        *clusters);
  }
