...
auto main(int argc, char *argv[]) -> int {
  ...
  typename Config::FramePackage<typename Config::DetectorData, ConcreteAcc>
    data(fc->loadMaps<typename Config::DetectorData, ConcreteAcc>("../../../data_pool/px_101016/Insu_6_tr_1_45d_250us__B_000000_3.dat",
  ...
  //    "/media/storage/schenk24/sum.bin",
  //    std::ios_base::binary);
  //
  std::size_t sums_written = 0;
  //
  //if (!sum_file.is_open()) {
  //  std::cerr << "Couldn't open sum output file!\n";
  //  abort();
  //}
  //
  //std::ofstream maxValues_file(
  //    "/media/storage/schenk24/maxValues.bin",
  //    std::ios_base::binary);
  //
  //if (!maxValues_file.is_open()) {
  //  std::cerr << "Couldn't open maximum value output file!\n";
  //  abort();
  //}
  //
  //std::ofstream energy_file(
  //    "/media/storage/schenk24/energy.bin",
  //    std::ios_base::binary);
  //
  ...
  //}
  //
  //std::ofstream cluster_energy_file(
  //    "/media/storage/schenk24/pede_update/cluster_energy.bin",
  //    std::ios_base::binary);
  //
  //if (!cluster_energy_file.is_open()) {
  //  std::cerr << "Couldn't open cluster energy output file!\n";
  //  abort();
  //}
  //
  //std::ofstream photon_file(
  //    "/media/storage/schenk24/photon.bin",
  //    std::ios_base::binary);
  //if (!photon_file.is_open()) {
  //  std::cerr << "Couldn't open photon output file!\n";
  //  photon_file.flush();
  //  photon_file.close();
  //  abort();
  //}
  ...
  typename Config::InitPedestal *pedePtr = alpakaNativePtr(initPed.data)->data;
  ...
  while (downloaded < data.numFrames) {
    ...
    if (currently_downloaded_frames) {
      ...
      // synchronize
      std::get<1>(downloadFutures.back()).wait();

      
      sums_written += (currently_downloaded_frames + Config::SUM_FRAMES - 1) /
                      Config::SUM_FRAMES;

      DEBUG("Writing sums",
            (currently_downloaded_frames + Config::SUM_FRAMES - 1) /
                Config::SUM_FRAMES,
            "/", sums_written, "(",
            sizeof(typename Config::SumMap) *
                ((currently_downloaded_frames + Config::SUM_FRAMES - 1) /
                 Config::SUM_FRAMES),
            "Bytes, single:", sizeof(typename Config::SumMap), "Bytes)");
      
      // save sum data if available
      //if (sum)
      //  sum_file.write(
      //      reinterpret_cast<char *>(alpakaNativePtr(sum->data)),
      //      sizeof(typename Config::SumMap) *
      //          ((currently_downloaded_frames + Config::SUM_FRAMES - 1) /
      //           Config::SUM_FRAMES));
      //
      // save max value data if available
      //if (maxValues)
      // maxValues_file.write(
      //      reinterpret_cast<char *>(alpakaNativePtr(maxValues->data)),
      //      sizeof(typename Config::EnergyValue) * currently_downloaded_frames);
      //
      // save photon data if available
      //if (photon)
      //  photon_file.write(
      //      reinterpret_cast<char *>(alpakaNativePtr(photon->data)),
      //      sizeof(typename Config::PhotonMap) * currently_downloaded_frames);
      //
      // save energy data if available
      //if (energy)
      //  energy_file.write(
      //      reinterpret_cast<char *>(alpakaNativePtr(energy->data)),
      //      sizeof(typename Config::EnergyMap) * currently_downloaded_frames);

      // save cluster energy data if available
      //if (energy)
      //  cluster_energy_file.write(
      //      reinterpret_cast<char *>(alpakaNativePtr(energy->data)),
      //      sizeof(typename Config::EnergyMap) * currently_downloaded_frames);
        ...
        std::get<1>(downloadFutures.back()).wait();
        if (energy)
          save_image<Config>("energy_" + std::to_string(i), alpakaNativePtr(energy->data), 0);
        
        save_image<Config>(
            "pedestal_" + std::to_string(i),
            alpakaNativePtr(dispenser.downloadPedestaldata().data), 0);
  ...
  save_image<Config>("pedestal_end",
                     alpakaNativePtr(dispenser.downloadPedestaldata().data), 0);
  
  // download and save std dev of the initial pedestal map
  /*auto initPed = dispenser.downloadInitialPedestaldata();
  //std::ofstream outPede("pede_stddev.bin", std::ios::binary);
  outPede.open("pede_stddev.bin", std::ios::binary);
  typename Config::InitPedestal * pedePtr = alpakaNativePtr(initPed.data)->data;
  for (unsigned int y = 0; y < Config::DIMY; ++y)
    for (unsigned int x = 0; x < Config::DIMX; ++x)
      outPede.write(reinterpret_cast<char *>(&(pedePtr[y * 1024 + x].stddev)),
  ...
  */
  ...
  // flush photon and energy data
  //photon_file.flush();
  //photon_file.close();
  //maxValues_file.flush();
  //maxValues_file.close();
  //sum_file.flush();
  //sum_file.close();
  //energy_file.flush();
  //energy_file.close();
  //cluster_energy_file.flush();
  //cluster_energy_file.close();

  // save clusters (currently only used for debugging)
  if (clusters) {
    //saveClusters<Config, ConcreteAcc>("clusters.txt", *clusters);
    saveClustersBin<Config, ConcreteAcc>("/media/storage/schenk24/pede_update/clusters.bin", *clusters);
  }
