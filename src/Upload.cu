...
Uploader::Uploader(std::array<Gainmap, 3> gain,
    ...
    : gain(gain), pedestal(pedestal), dimX(dimX), dimY(dimY),
      dataBuffer(RINGBUFFER_SIZE), photonBuffer(RINGBUFFER_SIZE),
...
void Uploader::initGPUs()
{
    ...
    std::vector<Pedestalmap> pedestal_unsplitted(pedestal.begin(),
                                                 pedestal.end());
    DEBUG("Maps created");
    std::vector<std::vector<Gainmap>> gain_splitted =
        splitMaps<Gainmap>(gain_unsplitted, devices.size());
    std::vector<std::vector<Pedestalmap>> pedestal_splitted =
        splitMaps<Pedestalmap>(pedestal_unsplitted, devices.size());
    DEBUG("Maps splitted");
    ...
    DEBUG("# of maps: " << gain_splitted.size() << " : "
                        << pedestal_splitted.size());
    ...
    for (std::size_t i = 0; i < devices.size(); ++i) {
        ...
        HANDLE_CUDA_ERROR(cudaMalloc((void**)&devices[i].gain,
                                     dimX * dimY * sizeof(double) * 3));
        HANDLE_CUDA_ERROR(cudaMalloc((void**)&devices[i].pedestal,
                                     dimX * dimY * sizeof(uint16_t) * 3));
        HANDLE_CUDA_ERROR(
            cudaMalloc((void**)&devices[i].data,
                       dimX * dimY * sizeof(uint16_t) * GPU_FRAMES));
        HANDLE_CUDA_ERROR(
            cudaMalloc((void**)&devices[i].photons,
                       dimX * dimY * sizeof(uint16_t) * GPU_FRAMES));
...
void Uploader::uploadGainmap(struct deviceData stream)
{
    ...
    void* fts = malloc(128);
    DEBUG("cudaMemcpy(" << stream.gain << ", " << stream.gain_host.at(0).data()
                        << ", " << stream.gain_host.at(0).getSizeBytes() * 3
                        << ", cudaMemcpyHostToDevice);");
    HANDLE_CUDA_ERROR(cudaMemcpy(stream.gain, stream.gain_host.at(0).data(),
                                 stream.gain_host.at(0).getSizeBytes() * 3,
                                 cudaMemcpyHostToDevice));
    DEBUG("Done!");
}
...
void Uploader::uploadPedestalmap(struct deviceData stream)
{
    ...
    HANDLE_CUDA_ERROR(cudaMemcpy(
        stream.pedestal, stream.pedestal_host.at(0).data(),
        stream.pedestal_host.at(0).getSizeBytes() * 3, cudaMemcpyHostToDevice));
    DEBUG("Done!");
}
...
void Uploader::downloadGainmap(struct deviceData stream)
{
    ...
    /*
            ...
            HANDLE_CUDA_ERROR(cudaMemcpy(gain->data(), gain_device,
       gain->getSizeBytes(), cudaMemcpyDeviceToHost));*/
}
...
void Uploader::downloadPedestalmap(struct deviceData stream)
{
    ...
    /*
    HANDLE_CUDA_ERROR(cudaSetDevice(device));
    HANDLE_CUDA_ERROR(cudaMemcpy(pedestal->data(), pedestal_device,
    pedestal->getSizeBytes(), cudaMemcpyDeviceToHost));*/
}
...
bool Uploader::calcFrames(std::vector<Datamap>& data)
{
    ...
    std::vector<std::vector<Datamap>> data_splitted =
        splitMaps<Datamap>(data, devices.size());

    for (std::size_t i = 0; i < devices.size(); ++i) {
        if (!uploadToGPU(devices[i], data_splitted[i]))
            return false;
        calculate<<</*devices.size() * NODES_PER_GPU / 128*/ 1, 128,
                    3 * (sizeof(uint16_t) + sizeof(double)) * 128,
                    devices[i].str>>>(uint16_t(dimX * dimY / devices.size()),
                                      devices[i].pedestal, devices[i].gain,
                                      devices[i].data, uint16_t(GPU_FRAMES),
                                      devices[i].photons);
        CHECK_CUDA_KERNEL;
                               ...
                               std::vector<Photonmap>& data)
{
    ...
    DEBUG("cudaMemcpyAsync(" << photonData << ", " << dev.photons << ", "
                             << numPhotons * sizeof(uint16_t)
                             << ", cudaMemcpyDeviceToHost, " << dev.str
                             << ");");

    HANDLE_CUDA_ERROR(cudaSetDevice(dev.device));
    HANDLE_CUDA_ERROR(cudaMemcpyAsync(photonData, dev.photons,
                                      numPhotons * sizeof(uint16_t),
                                      cudaMemcpyDeviceToHost, dev.str));
