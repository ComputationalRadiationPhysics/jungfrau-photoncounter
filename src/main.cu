        ...
        while ((current_offset = up.upload(data, current_offset)) <
            ...
            while (!up.isEmpty()) {
                ...
                if (is_first_package == 1) {
                    ...
                    save_image<Datamap>(std::string("dtest1000.bmp"), ready,
                                        std::size_t(0));
                    save_image<Datamap>(std::string("dtest1001.bmp"), ready,
                                        std::size_t(1));
                    save_image<Datamap>(std::string("dtest1002.bmp"), ready,
                                        std::size_t(2));
                    save_image<Datamap>(std::string("dtest1003.bmp"), ready,
                                        std::size_t(3));
                    save_image<Datamap>(std::string("dtest1004.bmp"), ready,
                                        std::size_t(4));
                    save_image<Datamap>(std::string("dtest1005.bmp"), ready,
                                        std::size_t(5));
                    save_image<Datamap>(std::string("dtest1006.bmp"), ready,
                                        std::size_t(6));
                    is_first_package = 0;
                }
                free(ready.data());
